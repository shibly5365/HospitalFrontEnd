...
export default function EditPatient({ patient, onClose, onSuccess }) {
    ...
    if (patient) {
      setFormData({
        ...
        emergencyContact: {
          ...
          number: patient.emergencyContact?.number || "",
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        notify.success("Patient information updated successfully!");
        if (onSuccess) onSuccess(res.data.data);
        if (onClose) onClose();
      }
    } catch (err) {
      console.error("Error updating patient:", err);
      notify.error(err.response?.data?.message || "Failed to update patient");
    } finally {
      setLoading(false);
    }
  };
